# ⚛️ @deflex.net/qvm-scheduler

> High-performance quantum circuit scheduler for multi-job quantum computing with OpenQASM 3

[![npm version](https://img.shields.io/npm/v/@deflex.net/qvm-scheduler.svg?style=flat-square)](https://www.npmjs.com/package/@deflex.net/qvm-scheduler)
[![npm downloads](https://img.shields.io/npm/dm/@deflex.net/qvm-scheduler.svg?style=flat-square)](https://www.npmjs.com/package/@deflex.net/qvm-scheduler)
[![license](https://img.shields.io/npm/l/@deflex.net/qvm-scheduler.svg?style=flat-square)](https://github.com/flexnetos/deflex.net/blob/main/LICENSE)
...
`@deflex.net/qvm-scheduler` is a high-performance Node.js native addon that provides TypeScript bindings for the [`qvm-scheduler`](https://crates.io/crates/qvm-scheduler) Rust crate. Built with [napi-rs](https://napi.rs/), it delivers native performance with a modern JavaScript API.
...
- **License**: MIT
- **Rust Crate**: [qvm-scheduler](https://crates.io/crates/qvm-scheduler)
...
```bash
npm install @deflex.net/qvm-scheduler
```

### Using yarn

```bash
yarn add @deflex.net/qvm-scheduler
```

### Using pnpm

```bash
pnpm add @deflex.net/qvm-scheduler
```
...
- **Linux x64**: `@deflex.net/qvm-scheduler-linux-x64-gnu`
- **Linux ARM64**: `@deflex.net/qvm-scheduler-linux-arm64-gnu`
- **macOS x64**: `@deflex.net/qvm-scheduler-darwin-x64`
- **macOS ARM64**: `@deflex.net/qvm-scheduler-darwin-arm64`
- **Windows x64**: `@deflex.net/qvm-scheduler-win32-x64-msvc`
- **WebAssembly**: `@deflex.net/qvm-scheduler-wasm32` (fallback)
...
```typescript
import { QvmScheduler } from '@deflex.net/qvm-scheduler'

// Create an instance
const client = new QvmScheduler({
...
```typescript
import { QvmScheduler, Config } from '@deflex.net/qvm-scheduler'
...
const client = new QvmScheduler(config)
```
...
// CommonJS
const { QvmScheduler } = require('@deflex.net/qvm-scheduler')
```
...
try {
  ...
  if (error instanceof QvmSchedulerError) {
...
Creates a new instance of `QvmScheduler`.

**Parameters:**
- `config` (optional): Configuration object

**Returns:**
- Instance of `QvmScheduler`
...
```typescript
class QvmSchedulerError extends Error {
  code: string
  details?: any
}
```
...
```typescript
import { QvmScheduler, QvmSchedulerError } from '@deflex.net/qvm-scheduler'
...
```typescript
import { QvmScheduler, Config, LogLevel } from '@deflex.net/qvm-scheduler'
...
cd deflex.net/packages/qvm-scheduler
...
- **NPM Package**: [@deflex.net/qvm-scheduler](https://www.npmjs.com/package/@deflex.net/qvm-scheduler)
- **GitHub Repository**: [flexnetos/deflex.net](https://github.com/flexnetos/deflex.net)
- **Rust Crate**: [qvm-scheduler](https://crates.io/crates/qvm-scheduler)
- **Documentation**: [docs.deflex.net/qvm-scheduler](https://docs.deflex.net/qvm-scheduler)
...
- Original crate by the [qvm-scheduler](https://crates.io/crates/qvm-scheduler) authors

---

## 📊 Stats

![NPM](https://nodei.co/npm/@deflex.net/qvm-scheduler.png?downloads=true&downloadRank=true&stars=true)
