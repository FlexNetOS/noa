...
impl CompressionManager {
    ...
    fn initialize_algorithms(&mut self) {
            ...
            .map_or(false, |c| c.enabled)
        {
            algorithms.insert(
                "zstd".to_string(),
                Box::new(algorithms::ZstdAlgorithm::new()),
            );
        }
            ...
            .map_or(false, |c| c.enabled)
        {
            algorithms.insert(
                "brotli".to_string(),
                Box::new(algorithms::BrotliAlgorithm::new()),
            );
        }
            ...
            .map_or(false, |c| c.enabled)
        {
            algorithms.insert(
                "gzip".to_string(),
                Box::new(algorithms::GzipAlgorithm::new()),
            );
        }
        ...
        if self.config.ml_optimization {
            ...
            ml_compressors.insert(
                "embedding".to_string(),
                Box::new(MLEmbeddingCompressor::new()),
            );
    ...
    pub async fn compress(&self, task: CompressionTask) -> Result<CompressionResult> {
        ...
        let result = if let Some(ml_type) = &task.ml_type {
            self.compress_ml_data(&task.data, ml_type, algorithm_name)
                .await?
        } else {
    ...
    pub async fn streaming_compress(
        &self,
        data_stream: impl futures::Stream<Item = Vec<u8>> + Send + 'static,
