    ...
    pub fn compress_stream(
        ...
        tokio::spawn(async move {
            let mut compressor = match algorithm.as_str() {
                ...
                _ => {
                    let _ = tx.send(Err(anyhow::anyhow!("Unsupported streaming algorithm: {}", algorithm))).await;
                    return;
                }
            };
...
impl StreamingLz4 {
    ...
    fn compress_chunk(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if self.buffer.len() >= 64 * 1024 { // 64KB chunks
...
mod tests {
    ...
    async fn test_streaming_compression() {
        ...
        let compressor = StreamingCompressor::new("zstd".to_string())
            .with_chunk_size(32); // Small chunk size for testing
        ...
        let decompressed_data: Vec<u8> = decompressed_chunks.into_iter()
