...
impl CacheCompressor {
    ...
    pub fn compress_with_priority(&self, data: &[u8], priority: CompressionPriority) -> Result<CompressionResult> {
        let algorithm = self.priority_algorithms.get(&priority)
            .unwrap_or(&"zstd");
...
impl AdaptiveCacheCompressor {
    ...
    pub fn compress_adaptive(&mut self, data: &[u8], priority: CompressionPriority) -> Result<CompressionResult> {
        // Based on performance history, choose the best algorithm
        let best_algorithm = self.select_best_algorithm(data, &priority);
        
        let start_time = std::time::Instant::now();
        let result = self.base_compressor.compress_with_priority(data, priority)?;
        let compression_time = start_time.elapsed().as_millis() as u64;
    ...
    fn select_best_algorithm(&self, _data: &[u8], priority: &CompressionPriority) -> &'static str {
        ...
        if recent_performance.is_empty() {
            return self.base_compressor.priority_algorithms.get(priority)
                .unwrap_or(&"zstd");
        }
