...
use crate::core::ui_generator::{UIComponent, ComponentType};
...
impl CodeGenerator {
    ...
    pub fn generate_component_code(&self, component: &UIComponent, platform: PlatformTarget) -> Result<String, CodeGenError> {
        match platform {
    ...
    fn generate_platform_adaptations(&self, component: &UIComponent) -> Result<String, CodeGenError> {
        ...
        adaptations.push_str(&format!(r#"
            // Platform-specific adaptations for {:?}
            div {{
                class: "platform-adaptive {}",
                
        "#, component.component_type, self.get_component_name(component))?);
    ...
    fn initialize_templates() -> HashMap<String, String> {
        ...
        // Add templates for different component types and platforms
        templates.insert(
            "button_web".to_string(),
            include_str!("../templates/button_web.rsx").to_string()
        );
        
        templates.insert(
            "text_web".to_string(),
            include_str!("../templates/text_web.rsx").to_string()
        );
