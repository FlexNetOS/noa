...
use crate::metadata::{RichMetadata, EntityType, MetadataQuery, MetadataValidationResult};
...
impl MetadataManager {
    ...
    pub async fn create_metadata(&self, entity_type: EntityType, created_by: String) -> Result<String> {
        let metadata = RichMetadata::new(entity_type, created_by);
        let id = metadata.id.clone();
        
        // Store metadata
        self.metadata_store.write().await.insert(id.clone(), metadata);
        
        // Update entity index
        self.entity_index.write().await
            .entry(entity_type)
            .or_default()
            .push(id.clone());
    ...
    pub async fn add_schema(&self, id: &str, schema: crate::metadata::schemas::SchemaDefinition) -> Result<()> {
        let mut store = self.metadata_store.write().await;
    ...
    pub async fn add_data_table(&self, id: &str, table: crate::metadata::data_tables::DataTable) -> Result<()> {
        let mut store = self.metadata_store.write().await;
    ...
    pub async fn track_lineage(&self, entity_id: &str, upstream_ids: Vec<String>, downstream_ids: Vec<String>) -> Result<()> {
    ...
    pub async fn get_entity_statistics(&self, entity_type: EntityType) -> EntityStatistics {
        ...
        EntityStatistics {
            ...
            average_quality: if count > 0 { total_quality / count as f64 } else { 0.0 },
            entity_ids,
        }
    }
    ...
    async fn matches_query(&self, metadata: &RichMetadata, query: &MetadataQuery) -> bool {
        ...
        if !query.tags.is_empty() {
            ...
            let has_matching_tag = metadata.data_tables.iter()
                .flat_map(|t| &t.metadata.tags)
                .chain(metadata.relationships.iter().flat_map(|r| r.properties.keys()))
        ...
        for relationship_query in &query.relationships {
            let has_matching_relationship = metadata.relationships.iter()
                .any(|r| {
                    r.target_id == relationship_query.target_id &&
                    relationship_query.relationship_type.as_ref().map_or(true, |t| &r.relationship_type == t)
                });
...
impl SchemaRegistry {
    ...
    pub fn register_schema(&mut self, schema: crate::metadata::schemas::SchemaDefinition) -> Result<()> {
...
impl LineageTracker {
    ...
    pub fn track_lineage(&mut self, entity_id: &str, upstream_ids: Vec<String>, downstream_ids: Vec<String>) {
        // Update upstream graph
        self.upstream_graph.insert(entity_id.to_string(), upstream_ids.clone());
        ...
        self.downstream_graph.insert(entity_id.to_string(), downstream_ids);
    ...
    pub fn get_impact_analysis(&mut self, entity_id: &str) -> Option<crate::metadata::ImpactAnalysis> {
        ...
        self.impact_cache.insert(entity_id.to_string(), analysis.clone());
        Some(analysis)
    }
    ...
    fn find_longest_path(&self, entity_id: &str, current_path: &mut Vec<String>, longest_path: &mut Vec<String>) {
...
impl ProvenanceTracker {
    ...
    pub fn track_transformation(&mut self, entity_id: String, steps: Vec<crate::metadata::TransformationStep>) {
