...
impl ProjectCompressionManager {
    ...
    pub async fn compress_project(
        ...
        let algorithm = self
            .compression_manager
            .get_best_algorithm(project_data)
            .await;
        let (compressed, stats) = self
            .compression_manager
            .compress_data(project_data, &algorithm)
            .await?;
    ...
    pub async fn decompress_project(
        &self,
        project_id: &str,
        compressed_data: &[u8],
        algorithm: &str,
        ...
        let decompressed = self
            .compression_manager
            .decompress_data(compressed_data, algorithm)
            .await?;
    ...
    pub async fn get_project_size(&self, project_id: &str) -> Option<(u64, u64)> {
        ...
        if let Some(data) = cache.get(project_id) {
            ...
            let stats = self
                .compression_manager
                .get_compression_stats(project_id)
                .await;
...
mod tests {
    ...
    async fn test_compression_algorithms() {
        ...
        for algorithm in &algorithms {
            ...
            let decompressed = manager
                .decompress_data(&compressed, algorithm)
                .await
                .unwrap();
    ...
    async fn test_project_compression() {
        ...
        let (compressed, stats) = project_manager
            .compress_project("test_project", project_data)
            .await
            .unwrap();
        ...
        let decompressed = project_manager
            .decompress_project("test_project", &compressed, &stats.algorithm)
            .await
            .unwrap();
