...
function Update-NoaEnvWithToolchains {
    ...
    if (Test-Path (Join-Path $nodeRoot "node.exe")) {
        $toolchainEnv += "`$env:npm_configs_prefix = `"$nodeRoot`""
        $toolchainEnv += "`$env:npm_configs_cache = `"$npmCache`""
    ...
    if ($toolchainEnv.Count -gt 0) {
        ...
        $envContent = @(
            ...
            ""
            "# Portable Toolchain configsuration"
...
try {
    ...
    $profileLines = @(
        ...
        '$env:NOA_configs = "$env:NOA_ROOT\configs"'
    ...
    Write-Log "Creating configs/noa.json..." -Level Info
...
'@
    ...
    $configsContent = $configsContent.Replace('TIMESTAMP_PLACEHOLDER', (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'))
    $configsContent = $configsContent.Replace('ROOT_PLACEHOLDER', $NoaRoot.Replace('\', '\\'))

    $configsContent | Set-Content -Path $configsPath -Encoding UTF8
    Write-Log "  Created: configs\noa.json" -Level Success
    ...
    Write-Log "configs File: $configsPath" -Level Info
    Write-Log "Log File: $script:LogFile" -Level Info
