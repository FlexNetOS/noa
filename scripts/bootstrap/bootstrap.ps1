...
.DESCRIPTION
    ...
    4. configsures kernel parameters (if applicable)
    5. Generates environment configsuration
...
.PARAMETER SkipKernel
    Skip kernel/networking configsuration
...
$directories = @(
    ...
    "configs", "repos", "workspace", "containers",
...
$cacheSetupScript = Join-Path $NOA_SCRIPTS "bootstrap/configs/cache-setup.ps1"
...
Write-Log "Phase 10: Generating environment configsuration..." -Level Info
...
# Node.js (portable)
`$env:npm_configs_prefix = "$(Join-Path $NOA_OPT 'node')"
`$env:npm_configs_cache = "$(Join-Path $NOA_OPT 'npm-cache')"
