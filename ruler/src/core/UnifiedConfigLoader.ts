...
export async function loadUnifiedconfigs (
  options: UnifiedLoadOptions,
): Promise<RulerUnifiedconfigs>
{
  ...
  if ( tomlRaw && typeof tomlRaw === 'object' )
  {
    ...
    if ( skillsSection && typeof skillsSection === 'object' )
    {
      ...
      {
        skillsconfigs = { enabled: skillsObj.enabled };
      }
    }
  }

  const toml: Tomlconfigs = {
    raw: tomlRaw,
    schemaVersion: 1,
    agents: {},
    defaultAgents,
  ...
  if ( tomlRaw && typeof tomlRaw === 'object' )
  {
    ...
    if ( tomlObj.mcp_servers && typeof tomlObj.mcp_servers === 'object' )
    {
      ...
      for ( const [ name, def ] of Object.entries( mcpServersRaw ) )
      {
        ...
        // Validate server configsuration
  ...
  // Store TOML MCP servers in toml configs
  ...
  const configs: RulerUnifiedconfigs = {
    meta,
    toml,
    rules,
    mcp,
  ...
  configs.hash = sha256(
    stableJson( {
      ...
      agents: Object.entries( configs.agents ).map( ( [ k, v ] ) => [ k, v.enabled ] ),
    } ),
  );
