...
/** Determine the native MCP configs path for a given agent. */
...
): Promise<string | null>
{
  ...
  switch ( adapterName )
  {
    ...
    case 'Cursor':
      ...
      break;
    case 'Windsurf':
      candidates.push( path.join( projectRoot, '.windsurf', 'mcp_configs.json' ) );
      break;
    case 'Claude Code':
      candidates.push( path.join( projectRoot, '.mcp.json' ) );
      break;
    case 'OpenAI Codex CLI':
      candidates.push( path.join( projectRoot, '.codex', 'configs.json' ) );
      break;
    case 'Aider':
      candidates.push( path.join( projectRoot, '.mcp.json' ) );
      break;
...
/** Read native MCP configs from disk, or return empty object if missing/invalid. */
...
/** Write native MCP configs to disk, creating parent directories as needed. */
