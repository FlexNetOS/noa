...
export async function propagateMcpToOpenHands (
  ...
  openHandsconfigsPath: string,
  backup = true,
): Promise<void>
{
  const rulerMcp: Record<string, unknown> = rulerMcpData || {};

  // Always use the legacy Ruler MCP configs format as input (top-level "mcpServers" key)
  ...
  try
  {
    const tomlContent = await fs.readFile( openHandsconfigsPath, 'utf8' );
    configs = parseTOML( tomlContent );
  } catch
  {
  ...
  if ( !configs.mcp.stdio_servers )
  {
    configs.mcp.stdio_servers = [];
  }
  if ( !configs.mcp.sse_servers )
  {
    configs.mcp.sse_servers = [];
  }
  if ( !configs.mcp.shttp_servers )
  {
    configs.mcp.shttp_servers = [];
  }

  // Build maps for merging existing servers
  const existingStdioServers = new Map<string, StdioServer>(
    configs.mcp.stdio_servers.map( ( s: StdioServer ) => [ s.name, s ] ),
  );

  const existingSseServers = new Map<string, string | RemoteServerEntry>();
  configs.mcp.sse_servers.forEach( ( entry: string | RemoteServerEntry ) =>
  {
  ...
  configs.mcp.shttp_servers.forEach( ( entry: string | RemoteServerEntry ) =>
  {
    const url = typeof entry === 'string' ? entry : entry.url;
  ...
  configs.mcp.stdio_servers = Array.from( existingStdioServers.values() );
  configs.mcp.sse_servers = normalizeRemoteServerArray(
    Array.from( existingSseServers.values() ),
  );
  configs.mcp.shttp_servers = normalizeRemoteServerArray(
    Array.from( existingShttpServers.values() ),
  );

  await ensureDirExists( path.dirname( openHandsconfigsPath ) );
  if ( backup )
  {
    const { backupFile } = await import( '../core/FileSystemUtils' );
    await backupFile( openHandsconfigsPath );
  }
  await fs.writeFile( openHandsconfigsPath, stringify( configs ) );
}
