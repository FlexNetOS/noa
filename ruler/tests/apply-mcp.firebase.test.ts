...
`;
    ...
    testProject = await setupTestProject( {
      ...
      '.idx/mcp.json': '{"mcpServers": {}}' // Empty native configs for Firebase Studio
    } );
  } );
  ...
  it( 'omits type fields in .idx/mcp.json', async () =>
  {
    ...
    expect( configs.mcpServers ).toHaveProperty( 'repo' );
    expect( configs.mcpServers.repo ).toEqual( {
      command: 'node',
      args: [ 'scripts/repo-mcp.js' ]
    } );

    expect( configs.mcpServers ).toHaveProperty( 'git' );
    expect( configs.mcpServers.git ).toEqual( {
      command: 'npx',
      args: [ '-y', '@modelcontextprotocol/server-git', '--repository', '.' ]
    } );

    // Ensure no 'type' property exists at server entries
    for ( const server of Object.values( configs.mcpServers ) )
    {
