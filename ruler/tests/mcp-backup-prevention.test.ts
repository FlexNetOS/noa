...
describe( 'MCP Backup Prevention for All Agents', () =>
{
  ...
  const mcpAgents = [
    { name: 'gemini-cli', configsPath: '.gemini/settings.json' },
    { name: 'copilot', configsPath: 'AGENTS.md' }, // Uses AgentsMdAgent + MCP handled by engine
    { name: 'claude', configsPath: 'CLAUDE.md' }, // Uses AbstractAgent + MCP handled by engine
    { name: 'cursor', configsPath: 'AGENTS.md' }, // Uses AgentsMdAgent + MCP handled by engine
  ];

  mcpAgents.forEach( ( { name, configsPath } ) =>
  {
    it( `should handle MCP correctly for ${ name } agent without creating backup files`, async () =>
    {
      ...
      const fullconfigsPath = path.join( projectRoot, configsPath );
      const backupPath = fullconfigsPath + '.bak';

      try { await fs.rm( fullconfigsPath, { force: true } ); } catch { }
      ...
      // Check that the main configs file exists
      const mainFileExists = await fs.access( fullconfigsPath ).then( () => true ).catch( () => false );
      expect( mainFileExists ).toBe( true );

      // For agents that write MCP data, check it exists
      if ( name === 'gemini-cli' )
      {
        const content = await fs.readFile( fullconfigsPath, 'utf8' );
        const configs = JSON.parse( content );
        expect( configs.contextFileName ).toBe( 'AGENTS.md' );
        expect( configs.mcpServers ).toBeDefined();
      }
