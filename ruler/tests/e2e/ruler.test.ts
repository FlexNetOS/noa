...
describe( 'End-to-End Ruler CLI', () =>
{
  ...
  beforeEach( async () =>
  {
    ...
    // Reset the TOML configs to default state
    ...
    await fs.rm( path.join( projectRoot, 'configs.toml' ), { force: true } );
    await fs.rm( path.join( projectRoot, '.kilocode' ), { recursive: true, force: true } );
  } );

  it( 'generates configsuration files for all agents', () =>
  {
    const { projectRoot } = testProject;
    ...
    const openHandsconfigsPath = path.join(
      projectRoot,
      'configs.toml',
    );
    ...
    return Promise.all( [
      ...
      .then( async () =>
      {
        const ohToml = await fs.readFile( openHandsconfigsPath, 'utf8' );
        const ohParsed: any = parseTOML( ohToml );
        expect( ohParsed.mcp.stdio_servers[ 0 ].name ).toBe( 'example' );
      } );
  }, 30000 );

  it( 'respects default_agents in configs file', async () =>
  {
  ...
  it( 'uses custom configs file via --configs', async () =>
  {
...
`;
    ...
    runRulerWithInheritedStdio( `apply --configs ${ alt }`, testProject.projectRoot );
    await expect(
      fs.readFile( path.join( testProject.projectRoot, 'custom_cursor.md' ), 'utf8' ),
    ).resolves.toContain( 'Rule A' );
  } );

  it( 'honors custom output_path in configs', async () =>
  {
    const toml = `
[agents.Copilot]
...
`;
  ...
  describe( 'gitignore integration', () =>
  {
    it( 'creates .gitignore with generated file paths by default', async () =>
    {
      ...
      expect( gitignoreContent ).toContain( 'configs.toml' );
    } );
    ...
    it( 'respects [gitignore] enabled = false in TOML configs', async () =>
    {
...
output_path = "custom-claude.md"`;
  ...
  describe( 'gitignore generation for all configss', () =>
  {
    ...
    it( 'should include all required file patterns in the generated .gitignore', async () =>
    {
      ...
      const expectedPatterns = [
        // MCP configs files (root-anchored)
        '/.vscode/mcp.json',
