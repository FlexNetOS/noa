...
describe( 'mcp-key-overwrite', () =>
{
  ...
  beforeEach( async () =>
  {
    ...
    // Create Copilot MCP configs using 'servers' key
    const copilotNative = { servers: { native_copilot_server: { url: 'http://copilot.com' } } };

    // Create Cursor MCP configs using 'mcpServers' key
  ...
  it( 'should overwrite with correct keys for different agents', async () =>
  {
    ...
    // Verify Copilot MCP configs was overwritten and uses 'servers' key
    ...
    // Verify Cursor MCP configs was overwritten and uses 'mcpServers' key
