...
`;
  ...
  it( 'should output type "http" for URL-based servers in Claude configs', async () =>
  {
    const { projectRoot } = testProject;

    runRulerWithInheritedStdio( 'apply --agents claude', projectRoot );

    // Verify Claude MCP configs uses 'http' type for URL-based servers
  ...
  it( 'should output type "sse" for SSE-based servers in Claude configs', async () =>
  {
    const { projectRoot } = testProject;
...
`;
    ...
    // Verify Claude MCP configs uses 'sse' type for SSE-based servers  
    const claudeResultText = await fs.readFile(
      path.join( projectRoot, '.mcp.json' ),
      'utf8',
    );
  ...
  it( 'should maintain correct structure and transformations when running for all agents', async () =>
  {
    ...
    // Verify Claude MCP configs has correct structure and transformations
    const claudeResultText = await fs.readFile(
      path.join( projectRoot, '.mcp.json' ),
      'utf8',
    );
