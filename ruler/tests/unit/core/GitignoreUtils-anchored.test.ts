...
describe( 'GitignoreUtils - Root Anchored Paths', () =>
{
  ...
  it( 'should anchor all paths to repository root with leading /', async () =>
  {
    ...
    expect( content ).toContain( '/.codex/configs.toml' );
    expect( content ).toContain( '/.aider/configs.yml' );
  ...
  it( 'should filter out paths inside .ruler directories', async () =>
  {
    const paths = [
      ...
      'subdir/.ruler/configs.toml',  // Should be filtered out
      '.codex/configs.toml'
    ];
    ...
    expect( content ).not.toContain( '.ruler/configs.toml' );
  } );
  ...
  it( 'should create specific backup patterns instead of broad wildcards', async () =>
  {
    ...
    expect( content ).toContain( '/.codex/configs.toml.bak' );
  ...
  it( 'should handle nested scenario with multiple .ruler directories', async () =>
  {
    ...
    const paths = [
      ...
      '.codex/configs.toml',                  // Root level MCP configs
      'subpkg/.codex/configs.toml',           // Nested MCP configs
    ...
    expect( content ).toContain( '/.codex/configs.toml' );
    expect( content ).toContain( '/subpkg/.codex/configs.toml' );
  ...
  {
    const paths = [ 'AGENTS.md', '.codex/configs.toml' ];
