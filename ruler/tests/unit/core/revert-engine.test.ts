...
import { IAgentconfigs } from '../../../src/core/configsLoader';
...
describe( 'revert-engine', () =>
{
  ...
  describe( 'revertAgentconfigsuration', () =>
  {
    it( 'should restore files from backup when backup exists', async () =>
    {
      ...
      const restoredContent = await fs.readFile( configsPath, 'utf8' );
    ...
    it( 'should remove generated files when no backup exists', async () =>
    {
      ...
      await fs.mkdir( path.dirname( configsPath ), { recursive: true } );
      await fs.writeFile( configsPath, 'generated content' );
      ...
      const fileExists = await fs.access( configsPath ).then( () => true ).catch( () => false );
    ...
    it( 'should handle dry run mode', async () =>
    {
      ...
      const currentContent = await fs.readFile( configsPath, 'utf8' );
