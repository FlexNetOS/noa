...
describe( 'CrushAgent', () =>
{
  ...
  it( 'should create CRUSH.md and .crush.json with ruler configs', async () =>
  {
  ...
  it( 'should update .crush.json with new mcp servers', async () =>
  {
    ...
    await agent.applyRulerconfigs( rules, projectRoot, newMcpJson );
  ...
  it( 'should not create .crush.json if no MCP configs provided', async () =>
  {
    const rules = 'some rules';
    ...
    // MCP file should not exist when no MCP configs is provided
  ...
  describe( 'backup and revert functionality', () =>
  {
    ...
    it( 'should be properly reverted by revertAllAgentconfigss', async () =>
    {
      const rules = 'Crush agent rules';
      const mcpJson = { mcpServers: { 'test-server': { command: 'echo' } } };

      await testAgent.applyRulerconfigs( rules, tmpDir, mcpJson );
      ...
      // Revert crush agent
      await revertAllAgentconfigss( tmpDir, [ 'crush' ], undefined, false, false, false );
    ...
    it( 'should handle custom output paths correctly', async () =>
    {
      ...
      await testAgent.applyRulerconfigs( rules, tmpDir, mcpJson, {
        ...
        outputPathconfigs: customMcpFullPath
      } );
    ...
    it( 'should properly merge existing MCP configsuration', async () =>
    {
      const mcpPath = path.join( tmpDir, '.crush.json' );

      // Create existing MCP configs
      const existingMcp = {
        mcp: {
      ...
      await testAgent.applyRulerconfigs( rules, tmpDir, newMcpJson );
