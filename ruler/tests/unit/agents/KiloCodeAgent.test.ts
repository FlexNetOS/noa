...
describe( 'KiloCodeAgent', () =>
{
  ...
  describe( 'File Operations', () =>
  {
    it( 'backs up and writes ruler_kilocode_instructions.md', async () =>
    {
      ...
      // Apply new configsuration
      await agent.applyRulerconfigs( 'new kilocode rules', tmpDir, null );
    ...
    it( 'creates directory structure if it does not exist', async () =>
    {
      ...
      // Apply configsuration without creating directory first
      await agent.applyRulerconfigs( 'kilocode content', tmpDir, null );
    ...
    it( 'uses custom outputPath when provided', async () =>
    {
      ...
      await agent.applyRulerconfigs( 'custom kilocode data', tmpDir, null, {
        outputPath: custom,
      } );
  ...
  describe( 'configsuration Overrides', () =>
  {
    it( 'respects custom output path in agent configs', async () =>
    {
      const agent = new KiloCodeAgent();
      const customPath = path.join( tmpDir, 'custom', 'kilocode_rules.md' );

      await agent.applyRulerconfigs( 'custom path content', tmpDir, null, {
