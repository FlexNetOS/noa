...
describe( 'GooseAgent', () =>
{
  ...
  it( 'should ignore MCP configsuration since Goose does not support local configs files', async () =>
  {
    const rules = 'Test instructions for Goose';
    const mcpconfigs = {
      mcpServers: {
    ...
    await agent.applyRulerconfigs( rules, tmpDir, mcpconfigs );
    ...
    // Should not create any configs files
    const configsPath = path.join( tmpDir, '.goose', 'configs.yaml' );
    await expect( fs.access( configsPath ) ).rejects.toThrow();
  } );
