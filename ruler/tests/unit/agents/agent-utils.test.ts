...
class MockMultiPathAgent implements IAgent
{
  ...
  getDefaultOutputPath ( projectRoot: string ): Record<string, string>
  {
    return {
      ...
      configs: path.join( projectRoot, '.mock.conf.yml' ),
...
describe( 'getAgentOutputPaths', () =>
{
  ...
  {
    it( 'returns default path when no configs is provided', () =>
    {
    ...
    it( 'returns custom outputPath when provided in configs', () =>
    {
      const agent = new MockSinglePathAgent();
      const agentconfigs: IAgentconfigs = {
        outputPath: '/custom/path/output.md',
      };

      const result = getAgentOutputPaths( agent, tmpDir, agentconfigs );

      expect( result ).toEqual( [ '/custom/path/output.md' ] );
    } );

    it( 'ignores other configs properties for single path agents', () =>
    {
      const agent = new MockSinglePathAgent();
      const agentconfigs: IAgentconfigs = {
        outputPath: '/custom/path/output.md',
        outputPathInstructions: '/ignored/instructions.md',
        outputPathconfigs: '/ignored/configs.yml',
      };
  ...
  {
    it( 'returns all default paths when no configs is provided', () =>
    {
    ...
    it( 'uses custom outputPathconfigs when provided', () =>
    {
      const agent = new MockMultiPathAgent();
      const agentconfigs: IAgentconfigs = {
        outputPathconfigs: '/custom/configs.yml',
      };
    ...
    it( 'uses both custom instruction and configs paths when provided', () =>
    {
      const agent = new MockMultiPathAgent();
      const agentconfigs: IAgentconfigs = {
        outputPathInstructions: '/custom/instructions.md',
    ...
    it( 'handles agent with no configs path', () =>
    {
      class MockNoconfigsAgent implements IAgent
      {
        getIdentifier (): string
        {
          return 'mock-no-configs';
        }

        getName (): string
        {
          return 'Mock No configs Agent';
        }
      ...
      const agent = new MockNoconfigsAgent();
      const result = getAgentOutputPaths( agent, tmpDir );
