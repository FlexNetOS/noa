...
describe( 'propagateMcpToOpenHands', () =>
{
  ...
  it( 'should create a new configs.toml with stdio_servers', async () =>
  {
    const rulerMcp = {
      mcpServers: { fetch: { command: 'uvx', args: [ 'mcp-fetch' ] } },
    };

    await propagateMcpToOpenHands( rulerMcp, openHandsconfigsPath );
  ...
  it( 'should merge servers into an existing configs.toml', async () =>
  {
    const rulerMcp = {
...
stdio_servers = [
  { name = "fs", command = "npx", args = ["mcp-fs"] }
    ...
    `;
    await fs.writeFile( openHandsconfigsPath, existingToml );
...
stdio_servers = [
  ...
  it( 'should propagate env variables for stdio servers', async () =>
  {
    ...
    const contentWithEnv = await fs.readFile( openHandsconfigsPath, 'utf8' );
  ...
  {
    await propagateMcpToOpenHands( null, openHandsconfigsPath );

    // Should not create configs file when data is null
    try
    {
      await fs.access( openHandsconfigsPath );
      // If file exists, it should be empty or not contain MCP configs
  ...
  it( 'should handle empty rulerMcp data gracefully', async () =>
  {
    ...
    // Should not create configs file when data is empty
    try
    {
      await fs.access( openHandsconfigsPath );
      // If file exists, it should be empty or not contain MCP configs
  ...
  it( 'should merge remote servers with existing OpenHands configs', async () =>
  {
    const rulerMcp = {
      mcpServers: {
