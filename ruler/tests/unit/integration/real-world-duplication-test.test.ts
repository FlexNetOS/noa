...
import { loadSingleconfigsuration, processSingleconfigsuration } from '../../../src/core/apply-engine';
...
describe( 'Real-world duplication bug reproduction', () =>
{
  ...
  it( 'should simulate the real CLI workflow with consecutive applies', async () =>
  {
    ...
    // Also create a basic ruler.toml configs
    await fs.writeFile( path.join( rulerDir, 'ruler.toml' ), '[agents.windsurf]\nenabled = true' );

    // Step 2: First apply - simulate `ruler apply`
    console.log( '\n=== FIRST APPLY ===' );
    let configs = await loadSingleconfigsuration( projectRoot, undefined, true );
    console.log( 'First apply concatenated rules length:', configs.concatenatedRules.length );
    console.log( 'First apply concatenated rules:', JSON.stringify( configs.concatenatedRules ) );

    const agents = resolveSelectedAgents( configs.configs, allAgents );
    ...
    await processSingleconfigsuration(
    ...
    console.log( '\n=== SECOND APPLY ===' );
    configs = await loadSingleconfigsuration( projectRoot, undefined, true );
    console.log( 'Second apply concatenated rules length:', configs.concatenatedRules.length );
    console.log( 'Second apply concatenated rules:', JSON.stringify( configs.concatenatedRules ) );
    ...
    expect( configs.concatenatedRules ).toBe( firstConcatenatedRules );
