...
describe( 'Test Harness', () =>
{
  describe( 'setupTestProject', () =>
  {
    ...
    it( 'creates nested directory structures', async () =>
    {
      const files = {
        ...
        '.ruler/subdir/configs.json': '{"test": true}'
      };
      ...
      const configsFile = path.join( testProject.projectRoot, '.ruler/subdir/configs.json' );

      expect( await fs.readFile( nestedFile, 'utf8' ) ).toBe( 'nested content' );
      expect( await fs.readFile( configsFile, 'utf8' ) ).toBe( '{"test": true}' );
    } );
  ...
  describe( 'runRuler', () =>
  {
    ...
    beforeEach( async () =>
    {
      // Create a basic test project with ruler configsuration
      testProject = await setupTestProject( {
