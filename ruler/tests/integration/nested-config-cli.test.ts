...
describe( 'CLI nested toggle precedence', () =>
{
  ...
  async function writeNestedProjectconfigs ( options: {
  ...
  it( 'activates nested processing when configs sets nested = true', async () =>
  {
    await writeNestedProjectconfigs( { nestedTomlValue: true } );
  ...
  it( 'remains flat when configs sets nested = false and CLI omits --nested', async () =>
  {
    await writeNestedProjectconfigs( { nestedTomlValue: false } );
  ...
  it( 'prefers CLI --nested over a configs that sets nested = false', async () =>
  {
    await writeNestedProjectconfigs( { nestedTomlValue: false } );
