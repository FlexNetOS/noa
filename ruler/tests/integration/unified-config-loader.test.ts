...
describe( 'UnifiedconfigsLoader integration', () =>
{
  const projectRoot = path.join( __dirname, 'fixtures', 'unified' );
  test( 'loads configs and rules (MCP normalization pending)', async () =>
  {
  ...
  test( 'loads nested configsuration option', async () =>
  {
    const unified = await loadUnifiedconfigs( { projectRoot } );
...
`;
    ...
    try
    {
      const unifiedWithNested = await loadUnifiedconfigs( { projectRoot } );
