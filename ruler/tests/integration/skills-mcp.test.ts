...
describe( 'Skills MCP Integration', () =>
{
  ...
  describe( 'buildSkillzMcpconfigs', () =>
  {
    it( 'creates MCP configs with absolute path', () =>
    {
      const configs = buildSkillzMcpconfigs( tmpDir );

      expect( configs ).toHaveProperty( 'skillz' );
      expect( configs.skillz ).toEqual( {
        command: 'uvx',
    ...
    it( 'uses absolute path even for relative project root', () =>
    {
      ...
      const configs = buildSkillzMcpconfigs( relativeRoot );

      const skillzPath = ( configs.skillz as any ).args[ 1 ];
